...
class PhotoMapViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, SHViewControllerDelegate {
    ...
    @IBAction func onSharePressed(_ sender: Any) {
        ...
        if (makingPost){
            ...
            myPicImage = resizeImage(image: myPicImage, targetSize: CGSize(width: 100, height: 100))
            myPic.image = myPicImage
            Post.uploadUserImage(image: myPic.image, withCaption: "") { (status: Bool, error: Error?) in
                
                if let error = error {
                    print (error.localizedDescription)
                } else {
                    print("I finished uploading prof pic")
                }
            }
            self.dismiss(animated: true, completion: nil)
